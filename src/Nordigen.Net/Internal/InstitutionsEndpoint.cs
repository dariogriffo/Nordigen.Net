...
namespace Nordigen.Net.Internal
{
    internal class InstitutionsEndpoint : IInstitutionsEndpoint
    {
        ...
        public Task<OneOf<Institution[], Error>> GetByCountryIso3166Code(string id, CancellationToken cancellationToken = default)
        {
            if (!CountryList.Contains(id))
            {
                throw new ArgumentException("Unknown country. Please check the official ISO country codes");
            }

            return _client.Get<Institution[]>($"api/v2/institutions/?country={id}", cancellationToken);
        }
    }
}
