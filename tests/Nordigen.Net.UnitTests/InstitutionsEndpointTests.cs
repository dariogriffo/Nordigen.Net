...
public class InstitutionsEndpointTests
{
    ...
    public async Task GetByCountryIso3166Code_When_Country_Is_Not_Found_Throws_ArgumentException()
    {
        ...
        handlerMock
            .When(HttpMethod.Get, $"/api/v2/institutions/?country={id}")
    ...
    public async Task Get_When_Id_Is_Found_Returns_Valid_Information()
    {
        ...
        const string payload =
            "{\"id\": \"ABNAMRO_ABNAGB2LXXX\",\"name\": \"ABN AMRO Bank Commercial\",\"bic\": \"ABNAGB2LXXX\",\"transaction_total_days\": \"540\",\"countries\": [\"GB\"],\"logo\": \"https://cdn.nordigen.com/ais/ABNAMRO_FTSBDEFAXXX.png\"}";
        handlerMock
            .When(HttpMethod.Get, $"/api/v2/institutions/{id}/")
            .With(x => x.Headers.Any(h => h.Key == "Authorization" && h.Value.First() == $"Bearer {authToken}"))
